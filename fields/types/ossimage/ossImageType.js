...
function ossimage(list, path, options) {
	...
	if (options.initial) {
		throw new Error('Invalid Configuration\n\n' +
			'ossimage fields (' + list.key + '.' + path + ') do not currently support being used as initial fields.\n');
	}
	...
	if (!options.oss_config || !options.oss_config.accessKeyId || !options.oss_config.secretAccessKey) {
		throw new Error('Invalid Configuration\n\n' +
			'ossimage fields (' + list.key + '.' + path + ') require the "oss_config.accessKeyId" and the "oss_config.secretAccessKey" option to be set.');
	}
...
util.inherits(ossimage, super_);


Object.defineProperty(ossimage.prototype, 'oss_config', { get: function() {
...
ossimage.prototype.pre = function(event, fn) {
	if (!this._pre[event]) {
		throw new Error('ossimage (' + this.list.key + '.' + this.path + ') error: ossimage.pre()\n\n' +
...
ossimage.prototype.post = function(event, fn) {
	if (!this._post[event]) {
		throw new Error('ossimage (' + this.list.key + '.' + this.path + ') error: ossimage.post()\n\n' +
			'Event ' + event + ' is not supported.\n');
	}
...
ossimage.prototype.addToSchema = function() {
	...
	var schemaMethods = {
		...
		/**
		 * Deletes the file from ossimage and resets the field
		 *
		 * @api public
		 */
...
ossimage.prototype.format = function(item, i) {
...
ossimage.prototype.validateInput = function(data) {
...
ossimage.prototype.updateItem = function(item, data) {
...
ossimage.prototype.uploadFiles = function(item, files, update, callback) {
 ...
 * - `field.paths.upload` in `req.files` (uploads the file to ossimage)
 *
 * @api public
 */
...
exports = module.exports = ossimage;
